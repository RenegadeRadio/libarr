...
def test_author_book_edition_file_chain(session):
    ...
    edition = Edition(book_id=book.id, isbn13="9780385171683", publisher="Doubleday", format="EPUB")
    session.add(edition)
    session.flush()
...
def test_book_fts_reindex_and_search(session):
    ...
    book = Book(author_id=None, title="Dune", description="Desert planet politics.", year=1965)
    book.author = author
    session.add(book)
    session.flush()
    session.add_all(
